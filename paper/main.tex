% ==========================================================================
%  A Null Return Is Not a Null Alpha:
%  Risk Adjustment as a False-Discovery Channel
%
%  Finance-only. The machine learning half of the earlier draft is parked on
%  a separate branch and tag; nothing was deleted from the repository, only
%  from this document.
%
...
% Anonymous submission build: no option = anonymised authors + line numbers.
% Swap to [preprint] for arXiv or [final] for camera-ready, and restore the
% author block below, in the SAME commit that de-anonymises the URL.
\usepackage{neurips_2023}
% Non-anonymous preprint build; the repository URL in the abstract is the real
% one and the repository is public, so this is internally consistent as it
% stands.  For a double-blind submission, switch to
% \usepackage[anonymous]{neurips_2023} AND replace the URL, in the same commit.
...
Code: \url{https://anonymous.4open.science/r/benchmark-zoo}.
\end{abstract}
...
\input{tables/corrections_on_null}
...
\input{tables/survival}

\input{tables/n_sensitivity}

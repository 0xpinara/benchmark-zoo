...
the answer can be checked against ground truth, then runs the corrections with
the measured null instead of the textbook one.

The population is 19,380 long-short equity strategies built from the letters of
ticker symbols (Chen and Dim's high-throughput data). A strategy that buys firms
...
Raw mean-return t-statistics on that population are close to standard normal
(sd 1.03). Five-factor alpha t-statistics are not (sd 1.40), and 5.2 percent of
them cross |t| > 3 where a normal says 0.27 percent.
...
are real. They are just not interesting. The fix is the one data mining found
when it replaced confidence with lift: re-centre the statistic, or calibrate it
on the population you are actually searching.

Paper: [`paper/main.pdf`](paper/main.pdf), 21 pages, built from `paper/main.tex`
by `make paper`.
...
Python 3.9 or newer. numpy, pandas, scipy and statsmodels for the statistics;
torch and ogb only for the benchmark sweep, in the `ml` extra.
...
population in the domain you are working in, which is the point.

## Corrections

Twelve methods behind one interface. Each has a test against a published worked
example or a published property of the procedure, and a docstring naming the
equation it follows.
...
The first thing anyone who knows the area will check, so it goes here.

Finance metrics are averages over time, so the bootstrap resamples time blocks
(Politis and Romano 1994, block length by Politis and White 2004). Benchmark
metrics are averages over instances, so it resamples instances. Permutation
nulls flip the sign of whole blocks, with one sign vector shared across the
cross-section, so the dependence and the autocorrelation both survive.

Every model shares the same resampled instance set inside a replicate.
Resampling each model separately makes the columns independent by construction,
which inflates the null maximum and makes every correction too conservative.
`bzoo.resample.instance` still exposes the wrong version, as
...
Stages run on their own too, and each only reads what an earlier one wrote.
`make help` lists them. The sweep is the slow part: the three small graphs take
under an hour on eight cores, ogbn-arxiv a few hours.
...
```
src/bzoo/
  finance/     loading, the known-null partition, performance stats, factor models
  ml/          datasets, models and ablations, budget parity, null construction
...
writes the `.tex` the paper `\input`s, so no number is transcribed by hand.

## Two things I got wrong first

Both are dated in `DECISIONS.md`.

1. The indices in Chen and Dim's past-return signal names are quarters ordered
   oldest first, not month lags. Read wrong, twelve-month momentum comes out at
   t = -2.42. Read right it is +5.60, and every other known effect lands with
   the sign the literature predicts.
2. A population that is null for one statistic need not be null for a
   transformation of it. Between 29 and 41 percent of the cross-sectional
   variance of the ticker alphas comes from the factor exposures, so the alpha
   null has to come from a permutation that imposes `alpha = 0` given the
   loadings, not from the marginal spread.

## Licence and data

Code is MIT. Only derived statistics are redistributed. The mined returns and
the OSAP portfolios are published by their own authors and pulled by
...
The one data file kept here is
`data/raw/leaderboards/ogbn_arxiv_leaderboard.csv`, a dated transcription of a
public page, so the analysis still reproduces after the page changes. See
`DATASHEET.md`.
